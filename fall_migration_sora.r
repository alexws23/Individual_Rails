...
tag <- df %>% 
  ...
  filter(
    ...
    !(motusTagID == 57167 & recvDeployName %in% c("Golfo de Santa Clara - RV Park", "Scotch_Bush", "GMNP-Berry Hill")), #Lots of weird detections from this bird. Feels improbably that the bird ended up on the gulf on california. Also a short detection in E Ontario that I think was untrue. Detected sporadically from 6/06 to 10/27 in 2022 in FL but not sure I trust those fully. There are a few summertime records on eBird from nearby sites though. Relatively short run at GMNP Berry hill in september is probably false
  ...
  filter(recvDeployName %ni% c("Kent Island", "Triton2","Bois de la Roche 2", "Bois de la Roche 1", "McGill_Bird_Observatory")) #these towers are clearly bad. Keep coming up in nonsensical ways
